...
async def _serve_both(host: str, port: int) -> None:
    main_srv = uvicorn.Server(uvicorn.Config("server:app", host=host, port=port, reload=False))
    view_srv = uvicorn.Server(uvicorn.Config("viewer:app", host=host, port=port + 1, reload=False))
    ...
    main_srv.install_signal_handlers = lambda: None
    view_srv.install_signal_handlers = lambda: None
