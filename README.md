...
# 모델 API 인증용 Ed25519 키페어 (선택 — 무인증 신뢰 네트워크면 생략)
uv run scripts/gen_keypair.py orchestrator   # → keys/orchestrator.key / .pub
echo 'WEBCOMFY_KEY_NAME="orchestrator"' >> .env
...
`.env` 항목: `COMFY_BASE_URL` / `WEBCOMFY_KEY_NAME`(첫 실행 시딩용) · `HOST` / `PORT` · `LOCAL_MODELS_DIR`(기본 `local_models/`) · `OUTPUT_DIR`(뷰어, 기본 `output/`).
...
일부 블록에서 여러 후보를 함께 넣어야 할 때만 **복수 추출 (고급)**을 펼쳐 개수를 지정할 수 있다.
활성·양수 가중치 후보를 가중 비복원 추출해 선택 순서대로 연결하며, 가능한 후보보다 많이 요청하면
전부 선택한다. 기본값 `1`은 과거 단일 선택 코드와 RNG 소비가 동일하므로 기존 config와 임베디드
재현 데이터는 같은 master seed에서 이전과 같은 프롬프트·sampler seed를 만든다.
...
원격 ComfyUI 서버들을 `servers.json` 레지스트리(id·이름·base_url·서명 키 이름·활성 여부)로 관리한다. 첫 실행 시 `.env`의 `COMFY_BASE_URL` / `WEBCOMFY_KEY_NAME`이 `default` 서버로 자동 이관된다.
...
- **인증** — Ed25519 요청 서명. webcomfy에서 `uv run scripts/gen_keypair.py <name>`으로 `keys/<name>.key`(개인키) / `keys/<name>.pub`(공개키)를 만들고, `.pub`을 각 ComfyUI의 `custom_nodes/ComfyUI-Remote-Manager/` 루트에 복사하면(scp 등 — `setup_comfyui.sh`가 끝나면 안내 출력) 확장은 루트의 모든 `*.pub`를 신뢰 키로 인식해 서명을 검증한다(타임스탬프 ±300초 + 논스로 리플레이 차단). `.pub`가 하나도 없으면 무인증(신뢰 네트워크 전제). webcomfy에는 서버 등록 시 키 이름(`<name>`)을 넣는다. 구 `WEBCOMFY_MODELS_TOKEN` Bearer 방식은 폐기.
...
- **경로 템플릿** — `저장 폴더`(기본 `output`) 하위에 템플릿으로 파일명을 만든다. placeholder: `{date}`(YYYY-MM-DD) · `{time}`(HHMMSS) · `{datetime}` · `{seed}` · `{index}`(AFK 전역 순번) · `{cname}`(현재 구성 이름) · `{lora_triggers}`(실제로 선택된 conditional LoRA 트리거를 설정 순서대로 `_` 연결) · `{ext}`. 예: `{cname}/{lora_triggers}/{date}-{time}.{ext}`.
...
- **`servers.py`** — `servers.json`에 `ServerEntry`(id/name/base_url/key_name/enabled) 목록을 원자적으로 저장. 첫 실행 시 `.env`를 `default` 서버로 시딩. `default_server()` = 첫 활성 서버. 레거시 `token` 필드는 로드 시 제거.
