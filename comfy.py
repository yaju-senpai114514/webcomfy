...
def get_options() -> ComfyOptions:
    """Fetch dropdown options (models, loras, samplers, ...) from /object_info."""
    info: Any = requests.get(f"{COMFY_BASE_URL}/object_info", timeout=30).json()
...
def queue_prompt(workflow: Mapping[str, Any], client_id: str) -> str:
...
def run(workflow: Mapping[str, Any], labels: Mapping[str, str], on_event: EventSink) -> None:
